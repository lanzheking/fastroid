package com.honestwalker.androidutils.ImageSelector.crop;
...
public class GrallyAndPhotoUtils {
    ...
    public static Bitmap decodeBitmapInScale(Resources res, int resID, int reqSize) {
        InputStream in = getBmpInputStream(res, resID);
        BitmapFactory.Options options = getStreamBitmapOptions(in, reqSize);
    ...
    public static Size getImageSize(String imagePath) {
        ...
        BitmapFactory.Options options = new BitmapFactory.Options();
        options.inJustDecodeBounds = true;
        BitmapFactory.decodeStream(getBmpInputStream(imagePath), null, options);
        float bmpWidth = options.outWidth;
        float bmpHeight = options.outHeight;
        size.setWidth(bmpWidth);
        size.setHeight(bmpHeight);
        return size;
    }
